...
/*Função para empilhar {elem} na pilha {s}. O(1)*/
...
/*Função para desempilhar o elemento no topo da pilha {s}. O(1)*/
int pop (Stack *s) {
  if (empty(s)) {
...
/*Função que retorna mas não remove o primeiro elemento de {s}. O(1)*/
...
/*Função que verifica se a pilha {q} está vazia (true) ou não {false}. O(1)*/
int empty (Stack *s) {
  return (s->peek == 0);	
}
